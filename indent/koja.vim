...
let s:block_openers = '\v^\s*(priv\s+)?(fn|test|struct|enum|builtin|impl|extend|protocol|if|match|cond|for|loop|while|receive|after|else|when)>'
let s:arrow_end     = '\v-\>\s*(#.*)?$'
let s:dedent_kws    = '\v^\s*(end|else|after|when)>'
let s:single_line   = '\v<if>.*<end>\s*$'
